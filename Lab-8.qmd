...
Using an independent t-test will allow us to see if there are differences among groups such as:

1.  Do women interpret emotions better than men?

2.  Do religious individuals give more to charity than their non-religious counterparts?

3.  Do people rate violent individuals as more angering than racist individuals?
...
# Create a vector of 25 ratings with an average of 4.7 and standard deviation of 2
Gaby_Garden <-round(rnorm(25,4.7,2),digits=0)

# Ensure values are not less than 1 or greater than 5
Gaby_Garden[Gaby_Garden <1] <- 1
Gaby_Garden[Gaby_Garden > 5] <-5

# Create a vector of 25 ratings with an average of 3.1 and standard deviation of 4
...
# Calculate a one-tailed t-test 
t.test(Gaby_Garden,Kim_Garden,var.equal = TRUE,alternative = "greater")
```

> David can conclude that Gaby's garden has higher ratings of attractiveness compared to Kim's garden as evidenced by an independent samples t-test, *t(48), 2.78, p = .003*.

Here we can visualize this difference:
...
barplot(
  c(mean(Gaby_Garden),
    mean(Kim_Garden)),
  names.arg = c("Gaby's Garden", "Kim's Garden"),
  ylab = "Average Rating",
  xlab = "Garden",
  col = c("#8ECAE6","#FB8500"),
  ylim = c(0,5))
